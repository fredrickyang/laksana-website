...
export default function Form() {
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (formStatusDiv) {
      formStatusDiv.className = 'block text-center text-xs mt-4 text-green-600';
      formStatusDiv.textContent = 'Form submitted successfully!';
      setTimeout(() => {
        formStatusDiv.className = 'hidden';
      ...
      >
        ...
        <div className="p-[1px] bg-gradient-to-b from-neutral-700 to-neutral-900 shadow-sm mt-4">
          <button
            type="submit"
            id="submitBtn"
            className="w-full bg-neutral-900 text-white py-4 text-xs font-semibold tracking-wide hover:bg-neutral-800 transition-colors flex items-center justify-center gap-2 cursor-pointer"
          >
            KIRIM PESAN
            <svg
        ...
        <div id="formStatus" className="hidden text-center text-xs mt-4" />
      </form>
    </div>
  </div>
</section>

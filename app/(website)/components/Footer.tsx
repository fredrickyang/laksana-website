...
export default function Footer({ settings }: FooterProps) {
  ...
  const copyrightText = footer.copyrightText ||
    '© 2025 PT Bangun Laksana Persada. All rights reserved.';

  const email = contactInfo.email || 'contact@agungintiland.com';
  const phoneNumber = contactInfo.phoneNumbers?.[0]?.number || '(021) 5886000';
  ...
  const headOfficeAddress = getAddressText(
    contactInfo.headOfficeAddress,
    'Jl. Pantai Indah Selatan No.9 Blok DC, RT.9/RW.6, Kapuk Muara, Penjaringan, North Jakarta 14460'
  );
                ...
                {socialLinks.map((social: any, idx: number) => (
                  ...
                  >
                    {social.platformName === 'Instagram' ? (
                      ...
                      <span className="text-xs">{social.platformName?.[0]}</span>
                    )}
                  </a>
                ))}
              </div>

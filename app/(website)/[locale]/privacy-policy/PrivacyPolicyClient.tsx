...
export default function PrivacyPolicyClient({
  ...
  const hero = privacyPage?.hero;
  const heroTitle = hero?.title || "Privacy Policy";
  const heroImage = getMediaUrl(hero?.backgroundImage);
  const lastUpdated = privacyPage?.lastUpdated
    ? new Date(privacyPage.lastUpdated).toLocaleDateString("en-US", {
      year: "numeric",
      month: "long",
      day: "2-digit",
    })
  ...
  const contactEmail = privacyPage?.contactEmail;
      ...
      <div className="py-20 px-6 bg-gradient-to-b from-gray-50 to-white">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-12 pb-8 border-b-2 border-gray-200">
            ...
            {privacyPage?.introText && (
              <div className="mt-4 text-lg">
                <RichTextContent data={privacyPage.introText} />
              </div>
            )}
            {privacyPage?.highlightText && (
              <div className="mt-6 p-4 bg-blue-50 border-l-4 border-blue-600">
                <RichTextContent data={privacyPage.highlightText} />
              </div>
            )}
          </div>
